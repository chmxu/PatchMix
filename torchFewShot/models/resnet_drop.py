...
class ResNet(nn.Module):
    ...
    def __init__(self, block, drop_block=False, drop_rate=0.0, dropblock_size=5, can=False):
        self.inplanes = 3
        ...
        if can:
            ...
            pool = False
            feats = [64, 160, 320, 640]
        self.nFeat = feats[-1]

        self.layer1 = self._make_layer(block, feats[0], stride=2, drop_rate=drop_rate)
        self.layer2 = self._make_layer(block, feats[1], stride=2, drop_rate=drop_rate)
        self.layer3 = self._make_layer(block, feats[2], stride=2, drop_rate=drop_rate, drop_block=drop_block, block_size=dropblock_size)
        self.layer4 = self._make_layer(block, feats[3], stride=2, drop_rate=drop_rate, drop_block=drop_block, block_size=dropblock_size, pool=pool)
    ...
    def forward(self, x, return_feat=False, return_both=False, return_map=False):
        ...
        #return [x1, x2, x3, x4]
        #return [x4]
        #return nn.AdaptiveAvgPool2d((1, 1))(x4)
        return x4
        #return [x3, x4]
        #return [x2, x3, x4]
...
def resnet12(can=False, drop_block=False, **kwargs):
    """Constructs a ResNet-12 model.
    """
    model = ResNet(BasicBlock, drop_block=drop_block, can=can, **kwargs)
    return model
